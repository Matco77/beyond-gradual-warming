# Appendix E — Coupling the AMOC Signal onto the ISIMIP3b Baseline
...
impact models, the response functions of Appendix D. It documents the scenario
replay that Appendix D defers to. The construction follows the *delta (change)
method*: the climate
...
drive the European energy-demand and crop-yield response functions (Appendix D),
with every weather covariate rebuilt by the same construction used in estimation
(Appendix D.2), so that the estimated response is transportable to the
