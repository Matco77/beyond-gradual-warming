...
def monthly_regression(mean_da, target_da):
    ...
    # Mask var BEFORE dividing so no inf is ever created (no warning), then
    # set those cells' slope to 0. Defensible: stationary climatological offset.
    var_safe = var.where(np.abs(var) > 1e-12)          # dead cells -> nan
    b = (cov / var_safe).fillna(0.0).rename("slope")   # nan -> 0 slope
...
def main():
    ...
    # climatologies for the anomaly product
    pic_tasmax_clim = pic_tasmax.groupby("time.month").mean("time")
    pic_tasmin_clim = pic_tasmin.groupby("time.month").mean("time")

    # Materialize the SMALL fit results (12 months x grid) into memory ONCE.
    # Without this, dask re-reads all 501 piControl files on every downstream
    # op, exhausting RAM -> segmentation fault. These arrays are tiny; safe.
    print("  loading fit coefficients into memory ...")
    a_max = a_max.compute(); b_max = b_max.compute()
    a_min = a_min.compute(); b_min = b_min.compute()
    pic_tasmax_clim = pic_tasmax_clim.compute()
    pic_tasmin_clim = pic_tasmin_clim.compute()
    ...
    for label, tas_glob in HOSING_EXPERIMENTS.items():
        ...
        # Load the (small, single-file) hosing tas fully into memory. Keeps all
        # downstream math as plain numpy -- no lazy dask graph that can segfault
        # on netCDF write under python 3.14 / limited RAM.
        hos_tas = hos_tas.compute()

        # grid check -- regrid coefficients only if needed
        aM, bM, aN, bN = a_max, b_max, a_min, b_min
        cmaxc, cminc = pic_tasmax_clim, pic_tasmin_clim
        if grids_match(pic_tas, hos_tas):
            ...
            cmaxc, cminc = _snap(pic_tasmax_clim), _snap(pic_tasmin_clim)
        else:
            ...
            cmaxc = regrid_coeff_to(pic_tasmax_clim, hos_tas)
            cminc = regrid_coeff_to(pic_tasmin_clim, hos_tas)
        ...
        # anomalies vs piControl climatology (same as you did for tas/pr)
        anom_max = (hos_tasmax.groupby("time.month") - cmaxc).drop_vars("month", errors="ignore")
        anom_min = (hos_tasmin.groupby("time.month") - cminc).drop_vars("month", errors="ignore")
        write_var(pic_tasmax, anom_max, "tasmax",
                  os.path.join(OUTPUT_DIR, f"tasmax_Amon_EC-Earth3_{label}_anomaly.nc"))
        write_var(pic_tasmin, anom_min, "tasmin",
                  os.path.join(OUTPUT_DIR, f"tasmin_Amon_EC-Earth3_{label}_anomaly.nc"))
