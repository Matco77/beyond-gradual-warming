...
#   python 1.reconstruct_tasmin_tasmax_ECEarth3.py --selfcheck
# =====================================================================
...
PIC_TAS_GLOB    = os.path.join(PICONTROL_DIR, "tas_Amon_EC-Earth3_piControl_r1i1p1f1_gr_*.nc")
PIC_TASMAX_GLOB = os.path.join(PICONTROL_DIR, "tasmax_Amon_EC-Earth3_piControl_r1i1p1f1_gr_*.nc")
PIC_TASMIN_GLOB = os.path.join(PICONTROL_DIR, "tasmin_Amon_EC-Earth3_piControl_r1i1p1f1_gr_*.nc")

# Hosing experiments to process: label -> glob for its monthly tas file(s).
# NAHosMIP filenames carry no variant/grid label, so nothing to pin there.
HOSING_EXPERIMENTS = {
...
def write_var(template_da, data_da, varname, out_path):
    ...
    out.attrs["fit_source"] = (
        f"EC-Earth3 piControl r1i1p1f1 gr, "
