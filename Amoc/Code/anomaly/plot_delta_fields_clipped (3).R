...
for (vn in vars) {
  ...
  zlim <- c(-zmax, zmax); pal <- pal_for(vn)
  ...
  for (d in loaded) {
    ...
    image.plot(d$lon, d$lat, fdisp, zlim = zlim, col = pal,
  ...
  mtext(sprintf("Late-run (plateau) delta \u2014 %s [%s]   |   shared scale clipped at %.0fth pct = \u00B1%.2f,  blue = cooling/drying,  NW\u2212Med<0 = AMOC fingerprint",
                vn, unit, CLIP_Q * 100, zmax),
